...
#[get("/users/")]
async fn read_with_pagination(data: Data<AppState>, page: Query<Page>) -> impl Responder {
    info!("read_all");
...
#[post("/users/")]
async fn create(data: Data<AppState>, session: Session, user: Json<NewUser>) -> impl Responder {
    info!("create");
...
#[get("/users/{user_id}/")]
async fn read(data: Data<AppState>, session: Session, path: Path<Info>) -> impl Responder {
    info!("read");
...
#[delete("/users/")]
async fn delete(data: Data<AppState>, session: Session, path: Query<Info>) -> impl Responder {
    info!("delete");
