...
Defines how the `active` flag on channels governs processing. Channels carry an `active` flag that operators toggle through the UI; the scheduled sync worker processes only active channels, while inactive channels keep their stored data and status untouched. Toggling the flag takes effect on the next sync cycle.
