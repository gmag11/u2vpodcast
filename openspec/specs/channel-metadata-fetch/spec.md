# channel-metadata-fetch

## Purpose

Defines how the system fetches channel metadata (title, description, image) from the upstream host when creating a channel or refreshing its cover image. The fetch must not block the async runtime and must be time-bounded, so slow or hung upstreams never degrade the rest of the API.
