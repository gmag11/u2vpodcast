...
Defines the "last episode age" badge on channel cards in the Vue 3 SPA. The badge shows how old the channel's most recent episode is, derived from the channel's `last_date`, using truncated units. Cards for channels without a `last_date` show no badge.
