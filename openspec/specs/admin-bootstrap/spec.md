...
Admin account provisioning at startup. `admin_username` and `admin_password` in `config.yml` are optional: when both are present the system reseeds the single admin user from config; when both are absent it leaves the existing `users` table untouched and resolves credentials against stored rows. Passwords are stored only as argon2 hashes, never plaintext. Startup logs the active provisioning mode so operators can confirm which path ran.
