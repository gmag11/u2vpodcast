# auto-playlist-append

## Purpose

Defines the automatic append of freshly downloaded episodes to the end of the server-persisted playlist. When the background download worker persists a new episode, that episode is added to the playlist using the same semantics as the playlist API: appended at the end, duplicates rejected, and episodes discarded below the retention floor never appended. The append is best-effort and never blocks or breaks the download run.
