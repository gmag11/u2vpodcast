...
Episode playback SHALL be managed by a global audio player shared across the app. The `EpisodeCard` SHALL NOT own a private `<audio>` element or private playback state; instead it SHALL bind its controls (play/pause, seek, volume, speed) to the global player store. The app shell SHALL render the persistent bottom player. This replaces the previous per-card player behavior while using the `/media/{slug}/{yt_id}.original.mp3` media URL pattern for the original audio.
