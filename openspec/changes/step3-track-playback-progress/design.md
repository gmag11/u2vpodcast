...
The same mark path is reached from the step-2 long-press next control (`skipNext({ markCurrent: true })`), which stores `position_seconds` as the episode's duration exactly like completion. All marking goes through one shared `markListened()` helper in the store. The endpoint accepts `listened: false` as well — clearing the mark and `listened_at` — which the step-4 unmark flow uses with `position_seconds: 0`.
