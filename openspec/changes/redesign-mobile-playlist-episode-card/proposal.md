...
- Add a playlist-specific mobile presentation below the existing `sm` breakpoint: reduced drag affordance, compact image-based play/pause, horizontally scrolling bold title, smaller static channel name, duration, date, progress, and read-only state icons.
