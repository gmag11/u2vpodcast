...
- [x] 5.2 Manually verify desktop system controls on the available representative Chromium browser plus any available Firefox/Safari environment; record play, pause, next, previous, relative/absolute seek availability, metadata, position, queue boundaries, and logout behavior for each tested browser/OS.
	- Chromium 148 / Electron 42.8.1 on Windows: Media Session accepted play, pause, next, previous, seek forward, seek backward, and seek-to registrations. Invoking the registered handlers controlled the shared player; relative seeks honored +30 seconds and the 15-second fallback, and absolute seek reached the requested position. Published position state contained bounded duration, position, and playback rate values.
	- Queue checks passed: next was a no-op at an empty boundary, consumed one queued episode when available, and replaced title/channel/artwork metadata; previous restarted at zero beyond three seconds, was a no-op without history at zero, and restored the prior episode and progress from history within three seconds.
	- Logout checks passed: playback became inactive, metadata was cleared, every action was unregistered, and a handler captured before logout could not restart playback.
	- Application failure: starting "Cómo reinventarte con IA empezando desde cero" directly from its channel page published an empty Media Session artist instead of the visible channel title "Inteligencia Artificial". Episodes started from the playlist published "Rohde & Schwarz" correctly.
	- Environment limitation: the integrated Chromium browser does not expose the Windows media flyout/media-key surface for inspection. No Firefox or Safari environment was available, so OS-rendered control availability and those browser rows remain unverified.
- [x] 5.3 Manually verify mobile controls on available Android Chrome and iOS Safari or installed-PWA environments; record notification/lock-screen/headset behavior, metadata and artwork, background play/pause, next/previous, seek controls, queue boundaries, and logout behavior, explicitly distinguishing browser/OS display limitations from application failures.
