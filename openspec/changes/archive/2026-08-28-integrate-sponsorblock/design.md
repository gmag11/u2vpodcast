...
If normalized segments cover the complete original duration, no valid MP3 frame can be retained. Treat this as a processing failure: reject the new snapshot, preserve the last valid representation (or the original when none exists), and retry later. Generating replacement silence is forbidden because it would require re-encoding and would no longer preserve source audio.
