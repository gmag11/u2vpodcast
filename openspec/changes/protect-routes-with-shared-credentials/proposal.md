...
- **Code**: `src/handlers/mod.rs` (scope restructuring), `src/main.rs` (media files wrapped), new `src/utils/middleware.rs` (two middlewares), `src/utils/mod.rs` (exports), `src/models/error.rs` and `src/models/response.rs` (already expose `CustomResponse::new`; reused as-is).
