...
**Goals:**
- A visible draggable thumb on the progress track in all three compositions (compact, wide, expanded).
...
### 1. Shared `ProgressScrubber.vue` component for all three tracks
Extract the repeated progress-track markup (track, fill, SponsorBlock markers, chapter markers, thumb, tooltip) into one reusable component. Props: `progress` (0–100), `duration`, `markers` (sponsor + chapter), `dataTestId`. Emits `seek(seconds)`. All three compositions render it, keeping markup and drag logic in one place.
- **Why**: The three tracks are near-identical already; a shared component removes triplication of the drag/preview logic and keeps behavior consistent.
- **Alternative**: Add thumb + logic inline to each of the three templates. Rejected: duplicates logic and risks drift.
...
### 5. Compact track becomes interactive and gains the hit area
The compact track currently has no hit area and no seek. It will use the same `ProgressScrubber` component so it inherits the thumb, drag, tooltip, and click-to-seek. Its visible track stays thin but the component keeps the larger invisible hit area used by wide/expanded.
- **Why**: Reuse and consistency; satisfies the modified `persistent-audio-player` requirement that compact be scrubbable.
...
- **Touch + scroll conflict on compact track** → Vertical scrolling isn't a concern on a thin bottom bar, and `setPointerCapture` keeps horizontal drags responsive. If needed, `touch-action` can be set to prevent unintended gestures.
- **Accessibility** → The wide/expanded tracks keep `role="slider"`; keyboard-arrow scrubbing is intentionally out of scope (non-goal).

## Migration Plan

Pure frontend addition; no data migration. Deploy with the SPA build. Rollback: revert the component changes; wide/expanded return to click-only and compact returns to read-only (prior behavior). No backend version bump required.
