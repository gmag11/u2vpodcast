...
- [ ] 1.2 Add mutations: `removeFromQueue(episodeId)`, `clearQueue()`, `skipNext()` (shift + play), `playPrevious()` (pop `playStack` + play without re-seeding), and seed on play: when `list` provided replace `upNext`; when omitted keep it.
...
- [ ] 3.1 In `frontend/src/components/PersistentPlayer.vue` add previous and next buttons; disable previous when `playStack` is empty and next when `upNext` is empty; wire to `playPrevious`/`skipNext`.
...
- [ ] 5.1 Extend `frontend/src/stores/player.test.ts`: seed replaces queue; play tanpa list keeps queue; `removeFromQueue`/`clearQueue`; `skipNext`; `playPrevious` restores last played; advance pushes to `playStack`; stop clears and persists empty.
