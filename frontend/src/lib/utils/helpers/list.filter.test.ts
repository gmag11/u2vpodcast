...
test('multi-word query requires all words in any order', () => {
	expect(filterBySearchWords(items, 'linux tapas', haystack).map((i) => i.id)).toEqual(['def456']);
	expect(filterBySearchWords(items, 'tapas linux', haystack).map((i) => i.id)).toEqual(['def456']);
});
